...
export class MainHeaderComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.subscriptions = [
            ...
            this.api.onNewEvents.subscribe((events: any) => {
                this.updateEvents(events, this.apiFirstUpdate);
                this.apiFirstUpdate = false;
            }),
            this.api.onSessionError.subscribe((error: any) => {
                console.error("session error", error);
                this.apiFirstUpdate = true;
                this.sessionError = error;
            }),
            this.api.onCommandError.subscribe((error: any) => {
    ...
    private updateSession(session: any) {
        ...
        this.counters.running = session.modules ? session.modules.filter((m: any) => m.running).length : 0;
    ...
    private eventCacheKey(event: any) {
        let key = (event.tag || "") + "::";
    ...
    private eventHTML(event: any): string {
    ...
    private isTrackedEvent(event: any): boolean {
    ...
    private eventClass(event: any): string {
    ...
    private handleTrackedEvent(event: any, firstUpdate: boolean) {
    ...
    private updateEvents(events: any, firstUpdate: boolean = false) {
        this.sessionError = null;
        this.counters.events = events.length;
        this.counters.dns = events.filter((event: any) => event.tag === 'net.sniff.dns' || event.tag === 'net.sniff.https').length;
